...
+ Methods to model relationship between hemodynamics during surgery and outcomes including acute kidney injury (AKI) and length of stay [@fine_mapping]
...
hts %>% 
  filter(id == id[1]) %>% 
  mutate(rel_time = as.numeric(difftime(time, min(time), units = "hours")),
         cat_cpb = factor(cat_cpb, levels = c("pre", "intra", "post"),
                          labels= c("Pre", "Intra", "Post"))) %>% 
  pivot_longer(cols = c(val_MAP, val_CVP)) %>% 
  mutate(name = factor(name, labels = c("Central Venous Pressure", "Mean Arterial Pressure"))) %>% 
  ggplot(aes(x = rel_time, y = value, color = cat_cpb)) + 
  geom_step() + 
...
ggsurvplot(
   ...
   pval.coord = c(20, 0.5),
   legend = "bottom",
   legend.title = "Low MAP Low CVP Exposure Time",
   legend.labs = c("At least 5 minutes", "Less than 5 minutes"),
