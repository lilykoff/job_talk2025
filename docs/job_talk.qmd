...
data $$\begin{bmatrix} v_1(1) & v_1(2) & v_1(3) & v_1(4) \\
v_2(1) & v_2(2) & v_2(3) & v_2(4) \\
\end{bmatrix}
$$ 

acceleration matrix $$\begin{bmatrix} v_1(2) & v_1(3) & v_1(4) & v_1(3) & v_1(4) & v_1(4) \\
...
\end{bmatrix}
$$ 
lag acceleration matrix $$\begin{bmatrix} v_1(1) & v_1(1) & v_1(1) & v_1(2) & v_1(2) & v_1(3) \\
...
xdf %>% 
  ...
  scale_fill_viridis_c(option = "B", name = "Mean daily steps per day, estimated by Self-Supervised Learner") +
  scale_x_continuous(limits=c(0,25000),
                     labels = seq(0, 30, 5),
                     breaks = seq(0, 30000, 5000)) +
  scale_y_discrete(labels = c("<18", "18-29", "30-39", "40-49", "50-59", "60-69", "70+")) +
  theme(legend.position = "none") + 
  labs(x = "Mean Steps per Day (x1000); Estimated by Self-Supervised Learner", y = "Age category") +
  annotate(
    geom = "label",
    x = 15000,
    y = 2,
    label = "Median daily steps <19 yrs: 9k",
    hjust = 0,
    color = "#444444"
  )  +
...
xdf %>% 
  ...
  labs(x = "Mean Steps per Day (x1000)", y = "",
...
+ Model: $$\mathbb{E}[\mathrm{steps}_i(s)] = \beta_0(s) + \beta_1(s)\mathrm{sex}_i + \beta_2(s)\mathrm{age}_i $$ 
$i$: participant; 
...
+ For standard regression, well developed methods and software exist that take into accounts weights and correlation between clusters (e.g. $\texttt{svyglm}$, $\texttt{svycoxph}$) [@surveys]
