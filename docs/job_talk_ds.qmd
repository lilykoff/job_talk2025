...
format:
  revealjs:
    ...
    toc: false
    progress: true
    footer: tinyurl.com/koff-talk-ds
    hash: true
    overview: true
    code-overflow: wrap
...
### --- extra hemodynamics ---- # 
hts = read_rds(here::here("docs", "data", "hemo_timeseries_interp_post_exclusion.rds")) %>% 
  as_tibble() 
...
hemo_ts_plotdf = 
  hts %>% 
  filter(id == id[1]) %>% 
  mutate(val_CVP = if_else(cat_cpb == "intra", NA_real_, val_CVP)) %>% 
...
hts1 = 
  hts %>% 
  filter(id == "10148_785999") %>% 
  mutate(val_CVP = if_else(cat_cpb == "intra", NA_real_, val_CVP)) %>% 
  mutate(time_elaps = as.numeric(difftime(time, min(time),
                               units = "hours"))) 


df_surv = read_rds(here::here("docs", "data", "temp_los_df.rds"))

aki_id = "14484_377496"
noaki_id = "75457_952257"
...
#### heart beat fpca ### 
beats_small = read_rds(here::here("docs", "data", "aligned_beats_sample.rds"))
fpca1 = read_rds(here::here("docs", "data", "fpca_res_1.rds"))
fpca2 = read_rds(here::here("docs", "data", "fpca_res_2.rds"))
fpca_res_all = read_rds(here::here("docs", "data", "fpca_res_allbeats.rds"))
...
- Can we apply these methods to other high-density data sources? 
...
- <span style="color:gray;">Can we apply these methods to other high-density data sources? </span>
...
plot_grid(p11, p_empty, ncol = 2, align = "v", rel_widths = c(1,1))
...
plot_grid(p11, p2, ncol = 2, align = "v", rel_widths = c(1,1))
...
plot_grid(p11, p3, ncol = 2, align = "v", rel_widths = c(1,1))
```
...
plot_grid(p11, p2b, p_empty, ncol = 3, align = "v", rel_widths = c(1, 1, 1))
...
### Big picture method: time series to images to scalar predictors

```{r print arrow x 2}

plot_grid(p11, p2b, p2a, ncol = 3, align = "v", rel_widths = c(1, 1, 1))
```
...
### Why not use image as predictor directly?
...
p3_bar = ggplot(dfProd, aes(t, prod)) + 
  geom_col(width = diff(range(t)) / length(t), fill = "#56B4E9", alpha = 0.8, color = "#444444") +
...
plot_ly(
  ...
  colorbar(title = "Density") %>%
  layout(
    title = list(text = "Predictors for one person", y = 0.95),
    # INCREASE margins here to prevent label clipping
    margin = list(l = 80, r = 80, b = 80, t = 0), 
    scene = list(
      xaxis = list(title = "Acceleration (g)"),
...
$$\text{logit}(p_{ij}^{i_0}) = \beta_0^{i_0} + \sum_{u=1}^{S} \sum_{s=u}^{S} F_{i_0} \{ v_{ij}(s), v_{ij}(s-u), u \} $$
</div>
...
$u = 1, \dots, S$ (index of lag length)

$s = u, \dots, S$ (index of current time point)

$v_{ij}(s)$ = value of acceleration for subject $i$, second $j$, at time index $s$
...
+ Train and test on session 1
  ...
  + <span style="color:#009E73;">Functional regression: 98 </span>

+ Train on session 1, test on session 2 
  + Logistic regression w/ grid cells: 41 
  + <span style="color:#009E73;">Functional regression: 53 </span>
:::
...
+ Train on session 1, test on session 2 
  ...
  + <span style="color:#009E73;">XGBoost w/ grid cells: 58</span>
...
# Other high density data sources: hemodynamics
...
+ Hemodynamics: how blood flows through your body
+ Modifiable risk factor for adverse outcomes after surgery, including acute kidney injury (AKI)
...
#### Minute-level hemodynamics and AKI 

Time series for two different CABG patients
...
start_end_df =
  hts %>% 
  filter(id %in% c(aki_id, noaki_id) & cat_anes == "intra") %>% 
  # mutate(val_CVP = if_else(cat_cpb == "intra", NA_real_, val_CVP)) %>% 
  group_by(id) %>% 
  mutate(start = min(time)) %>% 
  mutate(time_elaps = as.numeric(difftime(time, start,
                               units = "hours"))) %>% 
  summarize(start = min(time_elaps[cat_cpb == "intra"]),
            end = max(time_elaps[cat_cpb == "intra"])) %>% 
  ungroup() %>% 
  mutate(id2 = if_else(id == aki_id, "Developed AKI", "Did not develop AKI")) 
...
hts %>% 
  ...
  mutate(time_elaps = as.numeric(difftime(time, start,
                               units = "hours"))) %>% 
  mutate(id2 = if_else(id == aki_id, "Developed AKI", "Did not develop AKI")) %>% 
  pivot_longer(cols = c(val_MAP, val_CVP), names_to = "param", values_to = "value") %>% 
  mutate(param = factor(param, labels = c("CVP", "MAP"))) %>% 
  ggplot(aes(x = time_elaps, y = value, color = param)) + 
  facet_grid(param ~ id2, scales = "free_y") +
  labs(x = "Time since start of surgery (hours)", y = "Value (mmHg)") + 
  scale_color_manual(values = c("#56B4E9", "#009E73")) +
  theme(legend.position = "none") +
  geom_vline(data = start_end_df, aes(xintercept = start), color = "#444444", linetype = "dashed") +
  geom_vline(data = start_end_df, aes(xintercept = end), color = "#444444", linetype = "dashed") +
  geom_step(linewidth = 1) +
  geom_label(data = tibble(id2 = "Developed AKI", param = "MAP"),
            aes(x =  (2.72 + 3.98)/2, y = 130, label = "Bypass period"), color = "#444444") +
  geom_label(data = tibble(id2 = "Did not develop AKI", param = "MAP"),
            aes(x = (3.28 + 5.4)/2, y = 130, label = "Bypass period"), color = "#444444") +
  scale_x_continuous(breaks=seq(0, 7, 1)) 


```

<div style=" position: absolute; bottom: 0.5em; right: -4em; font-size: 0.7em; color: #777; "> @blood </div>

::: {.notes}
+ Mean arterial pressure (MAP): average pressure pushing blood through vessels, important for organ perfusion
+ Central venous pressure (CVP): pressure of blood returning to heart 
:::
...
### Arterial waveforms are patient-specific 
...
The XGboost model can identify the correct patient, which confirms that arterial waveforms are highly patient-specific, like walking patterns. 

But we're also interested in how patients are different from one another. One way to do that is to extract the underlying features of these beats.

::: 

---
 
### Arterial waveforms feature extraction
...
beats_small %>% 
  ...
  labs(x = "Time (seconds)", y = "Arterial Blood Pressure (mmHg)",
       title = "Aligned beats from 20 patients") +
  theme(legend.position = "none")
```
...
So here's a plot of the beats from 20 different patients, each color is a different patient. We want to find a lower dimensional representation of this data that preserves as much information as possible. We can do this with any autoencoder. 

We'll use principal components analysis here - which is just a single hidden layer autoencoder with a linear activation function. While using PCA might lose some nonlinear dynamics, it is easy to both compute and the eigenfunctions are physically interpretable. Let's look at the results! 
...
### Functional principal components: two patients 

```{r fpca1}
#| cache: true 

fpc1 = plot_fpca(fpca1, num_eigen = 3, n_plt = 12, pve = TRUE) +
  scale_x_continuous(limits=c(0, 120), breaks=seq(0,120,30), labels = seq(0, 1, 0.25)) +
  scale_y_continuous(limits = c(20, 200), breaks = seq(0, 200, 30)) +
  scale_color_manual(values = c("#D55E00", "#0072B2"), labels = c("High", "Low"), name = "Score") +
  theme(legend.position = c(.9, .8)) +
  labs(title = "First 3 PCs from Patient 1")

fpc2 = plot_fpca(fpca2, num_eigen = 3, n_plt = 12, pve = TRUE) +
  scale_x_continuous(limits=c(0, 120), breaks=seq(0,120,30), labels = seq(0, 1, 0.25)) +
  scale_y_continuous(limits = c(20, 200), breaks = seq(0, 200, 30)) +
  scale_color_manual(values = c("#D55E00", "#0072B2"), labels = c("High", "Low"), name = "Score") +
  theme(legend.position = c(.9, .8)) +
labs(title = "First 3 PCs from Patient 2")

fpc1 / fpc2 + plot_layout(guides = "collect", axes = "collect") & theme(legend.position = "bottom")
```

::: {.notes}
Here are first 3 PCs from two different patients
Orange curve: mean curve among beats in top 10% of PC1 scores (i.e. those with highest values of PC1)
Blue curve: mean curve among beats in bottom 10% of PC1 scores (i.e. those with lowest values of PC1)

::: 

---

### Functional principal components: all patients 

```{r fpca3}
#| cache: true 

plot_fpca(fpca_res_all, num_eigen = 3, n_plt = 25, pve = TRUE) +
  scale_x_continuous(limits=c(0, 120), breaks=seq(0,120,30), labels = seq(0, 1, 0.25)) +
  scale_y_continuous(limits = c(20, 200), breaks = seq(0, 200, 20)) +
  scale_color_manual(values = c("#D55E00", "#0072B2"), labels = c("High", "Low"), name = "Score") +
  theme(legend.position = "bottom") +
...
::: {.incremental}
+ Changes in fingerprint to predict changes in function (both walking and waveform)
+ Standardizing processing and analysis pipelines for wearable accelerometry 
+ Real-time prediction models for adverse outcomes during surgery
...
- Link to slides: [tinyurl.com/koff-talk-ds](tinyurl.com/koff-talk-ds) 
...
![](figs/koff-talk-ds-qr.png){width=150%}
...
### Sex differences in steps in NHANES 
...
hemo_res %>%
  ggplot(aes(
    ...
    y = cvp_fct,
    fill = estimate,
    label = format(estimate, digits = 2, nsmall = 2)
  )) +
  ...
  scale_fill_binned(
    ...
    breaks = c(
      ...
      1 / 0.5,
      1 / 0.4,
      round(max(hemo_res$estimate + .1), 1)
    )) +
  theme(legend.position = "none") +
  labs(
   x = "MAP Range (mmHg)",
    y = "CVP Range (mmHg)",
    title = "Odds Ratios of AKI",
...
res_plot = 
  ...
  mutate(estimate_multi2 = c(0.93, 1.01, 1.02, 1.03, 1.06)) %>% 
    mutate(num = sub(".*group\\_", "", group),
         group_lab = paste0("Zone ", num))

plotdf %>%
  mutate(num = sub(".*group\\_", "", group),
         group_lab = glue::glue("Zone ", num)) %>% 
  ggplot(aes(
    x = cut_MAP,
    y = cut_CVP,
    fill = group
  )) + geom_tile(col = "grey", linewidth = .5) +
  labs(
    x = "MAP Range (mmHg)",
    y = "CVP Range (mmHg)",
    fill = "Odds Ratio",
  ...
  geom_label(data = res_plot, 
          ...
          label = paste0(group_lab, "\n", "OR = ", estimate_multi2, "\n", pval_multi)),
             col = "black", hjust = 0, size = 5) +
  labs(title = "Odds Ratios of AKI",
    subtitle = "Associated with spending 5 additional minutes in given zone")

```

<div style=" position: absolute; bottom: 0.5em; right: -4em; font-size: 0.7em; color: #777; "> @fine_mapping </div>

::: {.notes}
Take something "simple" - time series - and make more complex (image) 
::: 

----

### Hemodynamics and length of stay 

```{r los}
#| cache: true 
# quartiles = quantile(df_surv$group_5, seq(0, 1, .25)); quartiles
df_surv = 
  df_surv %>% 
  mutate(los = los / 24, 
         lowmap_lowcvp_group = if_else(group_5 < 6, "<5 minutes", "5 or more minutes"))

fit = survfit(Surv(los, event) ~ lowmap_lowcvp_group, data = df_surv)
ggsurvplot(
   fit,       
   fun = "event",
   xlab = "Time (days)",
   ylab = "Hospital discharge probability",
   xlim = c(0, 40),
   ...
   pval.coord = c(20, 0.5),
   legend = "bottom",
   legend.title = "Zone 5 Exposure Time",
   legend.labs = c("Less than 5 minutes", "At least 5 minutes"),
...
### Hemodynamics publications

+ Fine-Mapping the Association of Acute Kidney Injury With Mean Arterial and Central Venous Pressures During Coronary Artery Bypass Surgery [@fine_mapping]
+ Occurrence of Low Cardiac Index During Normotensive Periods in Cardiac Surgery: A Prospective Cohort Study Using Continuous Noninvasive Cardiac Output Monitoring [@ci]
+ Assessment of Renal Vein Stasis Index by Transesophageal Echocardiography During Cardiac Surgery: A Feasibility Study [@rvsi]
+ Assessment of Renal Vein Flow Index by Transesophageal Echocardiography: Precision, Variability, and Association with Cardiac Index During Cardiac Surgery [@rvsi2]
+ Low Cardiac Index During Periods of Hypotension and Risk of Acute Kidney Injury in Cardiac Surgery. Goeddel, Koffman et al. 2025. *Under review*
+ Hemodynamic zones of arterial and venous pressure determine risk of kidney injury in cardiac surgery. Goeddel, Zhou, Hernandez, Koffman et al. 2025. *Under review*


::: {.notes}

What else is transportable? 
