...
p2b = arrow_plot + fprint_plot1 + plot_layout(widths = c(0.1, 0.9))
...
### Finding underlying features of arterial waveforms 
...
::: {.notes}
PCA: single hidden layer autoencoder with a linear activation function.

The principal components are the decoder weights, and the encoder is the pseudo-inverse of the principal components.

Autoencoders give you more flexibility (you get to play with architecture, activation functions etc.) but the optimization problem becomes harder so you may well end up with a less optimal encoder/decoder.

Best possible solution for a simple autoencoder (single hidden layer, linear activation). that is PCA. Easy to compute via linear algebra.

(Autoencoders leverage neural nets for nonlinear compression)
:::
...
### Sex differences in steps: complex survey function on scalar regression
