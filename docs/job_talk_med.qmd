...
title-slide-attributes:
    data-background-image: figs/koff-wfusm-url.png
...
$$
Y_i = \alpha + \sum_{k=1}^T \beta_k X_i(t_k)
$$
...
df_all = dd1 %>% mutate(lag = 0.01) %>% 
  bind_rows(dd2 %>% mutate(lag = 0.02)) %>% 
  bind_rows(dd50 %>% mutate(lag = 0.50)) %>% 
  bind_rows(dd99 %>% mutate(lag = 0.99))

# df_all = df_all %>%
#   mutate(lag_offset = scales::rescale(lag, to = c(0, 1)))

# ggplot(df_all, aes(x = vm, y = lag_vm + 3 * lag_offset, color = density)) +
#   geom_point(size = 0.9, alpha = 0.8) +
#   scale_color_viridis(option = "B", name = "Density") +
#   labs(
#     x = "Acceleration (g)",
#     y = "Lagged Acceleration (g)   +   Lag",
#     title = "Joint distribution across lag (stacked slices)",
#     subtitle = "Each sheet is a different lag — together forming a 3D predictor"
#   ) +
#   scale_x_continuous(limits = c(0, 3)) +
...
#   layout(
#     scene = list(
#       xaxis = list(title = "Acceleration"),
#       yaxis = list(title = "Lagged Acceleration"),
...
plot_ly(
  ...
  layout(
    ...
    title = "Predictors for one person and one second",
    scene = list(
...
plot_ly(
  ...
  layout(
    title = "Subset of predictors for one person, all seconds",
    margin = list(l = 30, r = 30, b = 30, t = 30),
    scene = list(
      xaxis = list(title = "Acceleration (g)"),
      yaxis = list(title = "Lag Acceleration (g)"),
...
# Working with other high density data sources
...
- Link to slides: [tinyurl.com/koff-wfusm](tinyurl.com/koff-wfusm) 
...
![](figs/koff-wfusm-qr.png){width=150%}
