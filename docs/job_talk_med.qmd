...
+ **Problem**: no method exists for function on scalar regression in complex survey settings

+ **Details of solution**: see Appendix
...
p1 = df2 %>% 
  ...
  scale_color_manual(values = c("#E69F00", "#009E73FF"), name = "Model",
                     labels = c("Unadjusted", "Survey-aware")) +
  theme(legend.position = "bottom")
p1
```
...
+ Modifiable risk factor for adverse outcomes after surgery
...
#### Pipeline for processing, analysis, and visualization

```{r hemo vis}
hemo_ts_plotdf %>% 
  mutate(cat_cpb = factor(cat_cpb, levels = c("pre", "intra", "post"),
                          labels= c("Pre", "Intra", "Post"))) %>% 
  ggplot(aes(x = time_elaps, y = value, color = cat_cpb)) + 
  geom_step() + 
  facet_wrap(param ~ ., scales = "free_y", nrow = 2) + 
  labs(x = "Time since start of surgery (hours)", y = "Value (mmHg)") + 
  scale_color_manual(values = c("#E69F00", "#56B4E9", "#009E73"), name = "Cardiopulmonary Bypass (CPB) Status") +
  theme(legend.position = "bottom",
...
#### Hemodynamic states and acute kidney injury (AKI)
...
hemo_ts_plotdf %>% 
  ...
  scale_color_manual(values = c("#000000", "#56B4E9"),
                     name = "Within recommended range (from literature)",
                     labels = c("Yes", "No")) +
    scale_x_continuous(breaks=seq(0, 7, 1)) 
...
#### Hemodynamic states and acute kidney injury
...
```{r our work recommends}
hemo_ts_plotdf  %>% 
  filter(!is.na(danger2)) %>% 
  ggplot(aes(x = time_elaps, y = value)) + 
  facet_wrap(param ~ ., scales = "free_y", nrow = 2) + 
  ...
  theme(legend.position = "bottom",
        ...
        legend.title = element_text(size = 14)) + 
  scale_color_manual(values = c("#000000", "#56B4E9"),
                     name = "Within recommended range (from our work)",
                     labels = c("Yes", "No")) +
    scale_x_continuous(breaks=seq(0, 7, 1)) 
...
#### Hemodynamics states and length of stay 
