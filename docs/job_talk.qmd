...
beta = sin(2*pi*t) # coefficient function
curve_data = tibble(t, beta)

curve_data %>% 
  ggplot(aes(t, beta)) + 
  geom_line(linewidth = 1.1, color = "#009E73") +
  labs(y = expression(beta(t)), x = "Time (t)",
       title = expression(Coefficient ~ Function ~ beta(s)))
...
p1 = ggplot(dfX, aes(t, X, color = id)) +
  ...
  labs(title = expression(Functional ~ Predictors ~ X[i](t)), y = "Value", x = "Time") +
...
p2 = ggplot(beta, aes(t, beta)) +
  ...
  labs(title =expression(Coefficient ~ Function ~ beta(t)), y = "Value", x = "Time") +
  geom_hline(aes(yintercept = 0), linetype = "dashed")+
  annotate(geom = "label", x = 0.3, y = 0.5, label = "Positive values of β(t) indicate\nthat higher values of X(t) increase\nthe outcome", size = 4, color = "#444444", vjust = 1) +
  annotate(geom = "segment", x= 0.3, xend = .3, y = 0.5, yend = 1.2,
           arrow = arrow(length = unit(0.05, "inches"), type = "closed"),
           color = "#444444")+
  annotate(geom = "label", x = 0.7, y = 0.5, label = "Negative values of β(t) indicate\nthat higher values of X(t) decrease\nthe outcome", size = 4, color = "#444444", vjust = 1) +
  annotate(geom = "segment", x= 0.7, xend = .7, y = 0, yend = -0.7,
           arrow = arrow(length = unit(0.05, "inches"), type = "closed"),
           color = "#444444")
...
p3 = ggplot(dfProd, aes(t, prod)) +
  ...
  labs(title = expression(Contribution ~ from ~ Subject ~ 1: ~  X[1]~(t)~x~ β~(t)),, y = "Value", x = "Time")
...
+ Model on the previous took 1D function input $X(t)$
...
+ Collaboration with cardiac anesthesiologists
+ Model hemodynamic values during surgery to prevent acute kidney injury [@fine_mapping]

:::: {.columns}

::: {.column width="50%"}

![](figs/cabg.jpeg){.equalheight}

:::

::: {.column width="50%"}

![](figs/cpb.jpeg){.equalheight}
